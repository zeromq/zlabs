...
(android_build_verify_so "libzmq.so" &> /dev/null) || {
    ...
    test -z "$ZMQ_ROOT" && ZMQ_ROOT="$(cd ../../../zmq && pwd)"
...
##
# Build zlabs from local source

(android_build_verify_so "libzlabs.so" "libzmq.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/zlabs"
    (cp -r ../.. "${cache}/zlabs" && cd "${cache}/zlabs" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/zlabs" && ./autogen.sh \
...
android_build_verify_so "libzlabs.so" "libzmq.so" "libczmq.so"
