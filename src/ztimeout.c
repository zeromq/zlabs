...
//  --------------------------------------------------------------------------
//  Helper to find node (set current)

static int
s_zring_goto (zring_t *self, ztimeout_node_t *node)
{
    void *n = zring_find (self, node);
    return n == node? 0: -1;
}
        ...
        if (self->next_time [i] <= now) {
            ...
            while (zring_size (list) > 0) {
                ...
                if (slot < 0) {
                    ...
                    zring_append (self->bucket [slot], node);
                    if (node->time < next) {
                        next = node->time;
                    }
                }
...
ztimeout_cancel (ztimeout_t *self, ztimeout_node_t *node)
{
    ...
    s_zring_goto (self->bucket [node->current_slot], node);
...
s_test_trigger (void **item_p)
{
    ...
    zsys_debug ("trigger at %ld", item->triggered);
}
